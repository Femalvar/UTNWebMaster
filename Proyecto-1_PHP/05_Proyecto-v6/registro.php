...
<?php
        ...
        // $ReclaveUser = $_REQUEST['ReclaveUser'];
        ...
        if($verificacion > 0){
            // ERROR el email ya esta regsitrado

            echo "<script>alert('El email corresponde a un usuario registrado.<br>Intentalo nuevamente.');document.location='index.php'</script>";
        }else{
            // consulta 2: alta (INSERT)
            $queryAlta = mysqli_query($conexion, "INSERT INTO usuarios VALUES(0, '$nombre','$apellido','$email', '$claveUser')");
            
            // if($queryAlta === true){
            //     echo "<p class='alert alert-success'>Gracias ".$nombre." por registrarse.</p>";
            // }  
        }
